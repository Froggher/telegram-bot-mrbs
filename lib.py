...
from telegram import ReplyKeyboardMarkup, Update, ReplyKeyboardRemove, InlineKeyboardButton, replymarkup
import re
import requests
...
AFTER_START, RIC_AVANZATA, MES_DESCRIPTION, DATA_MENU, DATA1_CALENDARIO, QUERY_ESAME, DOPO_RICERCA, QUERY_TIPO_CORSO, QUERY_AULA, PROF_MESSAGE, PRE_RICERCA,LIMIT_EXCEED,DATA2_CALENDARIO= range(13)
...
after_key = ReplyKeyboardMarkup(reply_keyboard2, one_time_keyboard=True, resize_keyboard=True, input_field_placeholder= "Risultati")


inline_keyboard = [
     
    [
        InlineKeyboardButton("Matematica I", callback_data='Matematica+I+Anno'),InlineKeyboardButton("Informatica I", callback_data='Informatica+I+Anno'),
    ],     
    [
        InlineKeyboardButton("Matematica II", callback_data='Matematica+II+Anno'),InlineKeyboardButton("Informatica II", callback_data='Informatica+II+Anno'),
    ], 
    [
        InlineKeyboardButton("Matematica III", callback_data='Matematica+III+Anno'),InlineKeyboardButton("Informatica III", callback_data='Informatica+III+Anno'),
    ], 
    [
        InlineKeyboardButton("Matematica Mag. I", callback_data='Matematica+Magistrale+I+Anno'),InlineKeyboardButton("Informatica Mag. I", callback_data='Informatica+Magistrale+I+Anno'),
    ],
    [
        InlineKeyboardButton("Matematica Mag. II", callback_data='Matematica+Magistrale+II+Anno'),InlineKeyboardButton("Informatica Mag. II", callback_data='Informatica+Magistrale+II+Anno'),
    ],
    [
        InlineKeyboardButton("Matematica lauree", callback_data='Matematica'),InlineKeyboardButton("Informatica lauree", callback_data='Informatica'),
    ],
    [
        InlineKeyboardButton("Altro", callback_data='Altro'),InlineKeyboardButton("↩️ Indietro", callback_data='Indietro')
    ],                           
...
inline_keyboard3 = [
    ...
    [
        InlineKeyboardButton("A0", callback_data='A0'),InlineKeyboardButton("A2", callback_data='A2'),InlineKeyboardButton("A3", callback_data='A3'),
    ],

    [
        InlineKeyboardButton("Aula C3", callback_data='Aula+C3'),InlineKeyboardButton("Aula Dott. V Piano - P1", callback_data='Aula+Dott.+V+Piano+-+P1'),InlineKeyboardButton("Aula Dott. V Piano - P2", callback_data='Aula+Dott.+V+Piano+-+P2'),
    ],      

    [
        InlineKeyboardButton("Aula Dott. V Piano - P3", callback_data='Aula Dott. V Piano - P3'),InlineKeyboardButton("Aula Gialla", callback_data='Aula+Gialla'),InlineKeyboardButton("Aula S - P1", callback_data='Aula+S+-+P1'),
    ], 

    [
        InlineKeyboardButton("Aula S - P2", callback_data='Aula+S+-+P2'),InlineKeyboardButton("Aula Verde", callback_data='Aula+Verde'),InlineKeyboardButton("B1", callback_data='B1'),
    ],      

    [
        InlineKeyboardButton("B3", callback_data='B3'),InlineKeyboardButton("C2", callback_data='C2'),InlineKeyboardButton("I1", callback_data='I1'),
    ], 

    [
        InlineKeyboardButton("I2", callback_data='I2'),InlineKeyboardButton("NB19", callback_data='NB19'),InlineKeyboardButton("NB20", callback_data='NB20'),
    ],      

    [
        InlineKeyboardButton("Portatile 4", callback_data='Portatile+4'),InlineKeyboardButton("Portatile 5", callback_data='Portatile+5'),InlineKeyboardButton("Portatile 6", callback_data='Portatile+6'),
    ], 
    
    [
        InlineKeyboardButton("Proiettore 4", callback_data='Proiettore+4'),InlineKeyboardButton("Sala Riunioni", callback_data='Sala+Riunioni'),InlineKeyboardButton("Indietro", callback_data='Indietro'),
    ], 
...
def stampa_risultato_inline(update: Update, context: CallbackContext) -> int:
    ...
    if len(flattened_events) == 0:
        context.bot.send_message(chat_id=update.callback_query.from_user.id,
                    text="Non sono state trovate prenotazioni.\nRiprovare la ricerca cambiando e/o aggiungendo parametri",reply_markup=after_key)
        return DOPO_RICERCA
    ...
    context.bot.send_message(chat_id=update.callback_query.from_user.id,
                        text="Sono state trovate " + str(len(flattened_events)) + " prenotazioni per i parametri impostati.",reply_markup=after_key)
...
def stampa_risultato_text(update: Update, context: CallbackContext) -> int:
    ...
    if update.message.text == '📌 Ricerca per oggi':
        ...
        if len(flattened_events) == 0:
            update.message.reply_text("Nessuna prenotazione per oggi",reply_markup=after_key)
            return DOPO_RICERCA

        update.message.reply_text("Sono state trovate " + str(len(flattened_events)) + " prenotazioni per i parametri impostati.\nUsa le frecce per cambiare aula e quella centrale per visualizzare i dettagli della prenotazione.",reply_markup=after_key)
        update.message.reply_text(event_visualizer(0, context), reply_markup = {True: None, False: create_button(0, context)}[len(context.chat_data['grouped_events']['uniquekeys'][0]) < 2])
        return DOPO_RICERCA
    ...
    if len(flattened_events) == 0:
        update.message.reply_text("Non sono state trovate prenotazioni.\nRiprovare la ricerca cambiando e/o aggiungendo parametri",reply_markup=after_key)
        return DOPO_RICERCA
    
    if len(flattened_events) > SHOW_MAX:
        update.message.reply_text("Il numero massimo di prenotazioni viusalizzabili è stato raggiunto")
        update.message.reply_text("Ritorno al menu",reply_markup=start_key)

        return AFTER_START

    context.chat_data['grouped_events'], lenght_key = group_events_by(flattened_events)

    update.message.reply_text("Sono state trovate " + str(len(flattened_events)) + " prenotazioni per i parametri impostati.",reply_markup=after_key)
    
    for lol in range(lenght_key):
        update.message.reply_text(event_visualizer(context=context, index=lol-1, detailed=False))
...
def prettify_event_detailed(event: dict) -> str:
    ...
    return "\n".join(["________________","📆 " + event["Evento"],"🗒 " + event["Descrizione"],"🔖 " + event["Tipo di prenotazione"],"✔️ " + event["Stato dell'approvazione"],"🚪 " + event["Sala"],"⏰ " + event["Ora Inizio"],
        "⏱ " + event["Durata"],"⌛️ " + event["Ora Fine"],"📇 " + event["Creato da"],"✏️ " + event["Modificato da"],"📜 " + event["Ultima Modifica"],"👤 " + event["Docente"],"🔏 " + event["E' un esame?"],"🎓 " + event["Corso di Studi"],
        "🔁 " + event["Tipo ripetizione"]

])

def group_events_by(event_list: list, grouping = "Sala") -> dict:
    groups = []
    uniquekeys = []
    #print(event_list)
...
def event_visualizer(index: int, context: CallbackContext, detailed=False) -> str:
    ...
    for i in range(len(context.chat_data['grouped_events']['groups'][index])):
        prettified_events.append({True: prettify_event_detailed(context.chat_data['grouped_events']['groups'][index][i]), False: prettify_event(context.chat_data['grouped_events']['groups'][index][i])}[detailed])

    return context.chat_data['grouped_events']['uniquekeys'][index] + "\n" + "\n".join(x for x in prettified_events) #prettify_event(context.chat_data['grouped_events']['groups'][index][0])
...
def event_visualizer_simple(context: CallbackContext, lenght: int) -> str:
    ...
    return context.chat_data['grouped_events']['uniquekeys'][lenght] + "\n" + "\n".join(x for x in prettified_events) #prettify_event(context.chat_data['grouped_events']['groups'][max_index][0])
...
def button(update: Update, context: CallbackContext) -> int:
    ...
    if action == "PREVIOUS":
        update.callback_query.edit_message_text(text = event_visualizer(int(index), context), reply_markup = create_button(int(index), context))
...
def crea_calendario(update: Update, context: CallbackContext)-> int:
    update.message.reply_text("Inserire data: ",
                        reply_markup=telegramcalendar.create_calendar())
    context.user_data['typecal'] = update.message.text
    return DATA1_CALENDARIO


def calendario_data1(update: Update, context: CallbackContext)-> int:
    query=update.callback_query
    query.answer()
...
def calendario_data2(update: Update, context: CallbackContext) -> int:
    query=update.callback_query
    query.answer()
    user_data = context.user_data
    a1 = user_data['cal1']
...
def mem_settimana(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Preferenza data memorizzata",reply_markup=start_key)
...
def find_event_id(url: str) -> str:
    ...
    id_list = []
    counter=0
    for i in re.findall('view\_entry\.php\?id\=\d+', r.text):
...
def event_query(a_date, b_date, areamatch = "", roommatch = "", typematch1 = "", typematch2 = "", namematch = "", descrmatch = "", creatormatch = "", match_private = "0", match_confirmed = "2", match_approved = "2", match_organizzatore = [], match_tipocorso = "", esame="",only_aula=False):
    ...
    """
    mrbs = 'https://servizi.dmi.unipg.it/mrbs/report.php?from_day=' + from_day + '&from_month=' + from_month + '&from_year=' + from_year + '&to_day=' + to_day + '&to_month=' + to_month + '&to_year=' + to_year + '&areamatch=' + areamatch + '&roommatch=' + roommatch + {True: '&typematch%5B%5D=E', False: ''}[typematch1 == "E"] + {True: '&typematch%5B%5D=I', False: ''}[typematch2 == "I"] + '&namematch=' + namematch + '&descrmatch=' + descrmatch + '&creatormatch=' + creatormatch + '&match_private=' + match_private + '&match_confirmed=' + match_confirmed + '&match_approved=' + match_approved + '&match_organizzatore='  + esame + '&match_tipocorso=' + match_tipocorso + '&output=0&output_format=0&sortby=r&sumby=d&phase=2&datatable=1&ajax=1&phase=2'
    event_list = []
    temp_list = []
    aula_list = []
    ...
    if only_aula==True:
        li1 = ["A0", "A2", "A3", "Aula C3", "Aula Dott. V Piano - P1", "Aula Dott. V Piano - P2", "Aula Dott. V Piano - P3",
...
def start_find_prof(update: Update, context: CallbackContext) -> int:
    if int(len(update.message.text)) < 3:
        update.message.reply_text("Errore inserire minimo 3 caratteri", reply_markup = adv_key)
        return PROF_MESSAGE
    ...
    if update.message.text == "/clear":
        update.message.reply_text("Ho reimpostato il filtro dei docenti", reply_markup = adv_key)
        return RIC_AVANZATA
    ...
    update.message.reply_text("Ho trovato " + str(len(prof)) + " professori che contengono \"" + stringa + "\":\n" + "\n".join(x for x in prof),reply_markup=adv_key)

    return RIC_AVANZATA



def pre_find_professor(update: Update, context: CallbackContext)-> int:
    update.message.reply_text("Digita il nome del docente che stai cercando.\nSe vuoi rimuovere i docenti impostati basta inviare /clear")
    return PROF_MESSAGE



def ric_avanzata(update: Update, context: CallbackContext) -> int:
...
def data_menu(update: Update, context: CallbackContext) -> int:
    update.message.reply_text(
...
def menu_text(update: Update, context: CallbackContext) -> int:
    update.message.reply_text(
    "Menu principale",
...
def menu_inline(update: Update, context: CallbackContext) -> int:
    query=update.callback_query
    query.answer()
    context.bot.send_message(chat_id=update.callback_query.from_user.id,
...
def breve_descrizione(update: Update, context: CallbackContext) -> int:
    update.message.reply_text("Inviare un messaggio contenente una breve descrizione")
    return MES_DESCRIPTION

def mem_descrizione(update: Update, context: CallbackContext)-> int:
    context.user_data["DESCRIZIONE"]=update.message.text
    update.message.reply_text("Scelta memorizzata")
    update.message.reply_text("Altri parametri",reply_markup=adv_key)
